...
class TrendDetection:
    ...
    def relative_comparison(self):
        ...
        plt.savefig(f'/home/egovridc/PycharmProjects/{self.kw_list[0]}.png', orientation='landscape')
    ...
    def yearly_topCharts(self):  # needs automation
        trend = self.pytrend.top_charts('2021', hl='en-US', tz=300, geo='GLOBAL')  # pytrend.suggestions('tanzania')
        print(trend)
        print(trend.to_json())


keywords = []  # remember limit is five except for check_trends()
for d in range(0, 5):
    dat = input('Enter up to 5 search terms: ')
    if dat:
        keywords.append(dat)
        print(keywords)
    else:
...
sFilter = input('Enter the type of search sFilter (i.e images, news, youtube, froogle): ')
if sFilter:
    print(sFilter)
else:
    sFilter = ''

timeSelection = input("Choose the timeframe index of your search (i.e all:0, last5yrs:1, last12months:2,...): ")
if timeSelection:
